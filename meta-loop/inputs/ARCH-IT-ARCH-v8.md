# META-LOOP ITERATION 3 (ARCH-IT-ARCH-v8) — INPUT

You are improving the KAIROS multi-agent pipeline architecture.

## What round 2 produced (and what to fix)

Round 2 ran end-to-end with these REAL findings:
1. Architect produced 5 architecture deliverables (~12KB each), but architecture.md was only a 1KB metadata pointer file referencing the other 4 — NOT the actual architecture content. Blind-tester (in 414K of real critique) flagged this correctly.
2. DevOps attempted v7.1 deploy and reported REAL BUILD FAILURE — proves devops engages with actual code but the build step needs hardening.
3. Blind-testers ran out of context before producing the final JSON review block (Hermes session exhausted at 414K chars output).
4. v7.1 dispatcher with idempotency keys + DLQ + envelope unwrap is LIVE on the cluster — verify in `/var/lib/karios/orchestrator/event_dispatcher.py` (size 128KB).

## Your task — produce v8.0 architecture proposal addressing:

A. ARCHITECTURE.MD MUST CONTAIN ACTUAL ARCHITECTURE CONTENT — not a manifest/pointer. Write the real system topology (9 agents, Redis streams, A2A, HITL, etc.), data flows, and interaction diagrams.

B. BLIND-TESTER OUTPUT FORMAT ENFORCEMENT — propose how to make the blind-tester profile produce JSON FIRST (before any prose) so the orchestrator never has to scrape free-text. Suggest profile prompt changes + JSON-fence-required fail-fast contract.

C. DEPLOY BUILD HARDENING — propose pre-deploy validation (go vet + golangci-lint + ast.parse for python + dry-run smoke test) so devops catches build failures BEFORE the deploy step runs.

D. SESSION-FLUSH POLICY VALIDATION — propose how to A/B test whether the new always-fresh-for-blind-testers policy actually improves review quality (validation harness comparing pre/post-flush ratings).

## You have full vault visibility now

The agent-worker now injects Cross-Agent Vault Context into your prompt. You also have CLI access:
  /usr/local/bin/karios-vault search "<keywords>"
  /usr/local/bin/karios-vault recent --kind rca --limit 5

Use these to inform your design with what the OTHER 8 agents have already learned.

## v6.0 architecture document (for reference)
---
