# META-LOOP ITERATION 4 (ARCH-IT-ARCH-v9) — INPUT

## Pipeline state (proven in rounds 1-3)

The KAIROS multi-agent pipeline runs on dispatcher v7.3 (live at /var/lib/karios/orchestrator/event_dispatcher.py).

Round 3 demonstrated:
- Phase 2 architect [ARCH-COMPLETE] sent NATURALLY
- Phase 3 backend + frontend [CODING-COMPLETE] both NATURAL, FAN-IN COMPLETE
- Phase 4 API-SYNC NATURAL
- Phase 5 devops attempted REAL deploy with REAL build failure documented
- Backend independently used vault visibility to work on BG-01 (CPU/RAM morphing)

Open issues from round 3 (you must address):
1. Architect's architecture.md is consistently a placeholder/manifest (1KB) — actual content goes into the OTHER 4 docs but architecture.md stays empty. v7.3 added a HARD PRE-SUBMIT GATE in the architect profile requiring all 5 files >=2KB before [ARCH-COMPLETE].
2. Blind-testers exhaust context before producing JSON (414K Hermes output, no JSON fence). v7.3 added STRICT OUTPUT CONTRACT in both blind-tester profiles requiring JSON FIRST in fenced block, total <30K chars.
3. Subject-format drift: agents inventing [ARCHITECTURE-COMPLETE], [BLIND-E2E-RESULTS], etc. v7.3 added handler aliases for these.
4. Telegram phase-transition notifications: v7.3 wired notify_phase_transition() into [ARCH-REVIEWED], [E2E-RESULTS], [STAGING-DEPLOYED], [PROD-DEPLOYED] handlers — Sai now gets a Telegram message at every phase transition with score + handoff target.
5. SOP precondition was checking output_files exist BEFORE dispatch — fixed; output_files now postcondition only.
6. load_gap returned phase=unknown — fixed with state.json fallback.

## Your task — produce v9.0 architecture proposal addressing what's STILL not solved:

A. **End-to-end test that proves all 6 phases pass naturally**: Design a self-test gap (e.g., "BG-stub-feature-no-op") that runs through Phase 1-6 with measurable success per phase. Pipeline becomes self-validating.

B. **Eliminate forging needed at any phase gate**: Identify why blind-testers + tester sometimes return non-JSON. Propose Hermes-side fix (shorter prompt? earlier JSON write? streaming output to disk?).

C. **Lock the message contract**: Propose a Pydantic-style schema for every cross-agent message (subject, body fields, idempotency). Agents validate before send. Orchestrator validates before processing. Reject-and-feedback loop on schema violations.

D. **Real production deploy of v7.3**: The current /var/lib/karios/orchestrator/event_dispatcher.py IS v7.3 already (verified by md5sum). Devops, when triggered, must verify and confirm rather than re-deploy. Propose the verification protocol.

E. **Vault as source of truth**: Test that agents actually USE the vault context that's now injected into their prompts. Propose a rubric: was a recent vault entry mentioned in the agent's output? Score per agent.

## You have full vault visibility
