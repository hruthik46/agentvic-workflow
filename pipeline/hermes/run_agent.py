...
class AIAgent:
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.tools:
            ...
            # v7.16 KAIROS patch: hard tool_choice for MiniMax + OpenAI-compat endpoints.
            # Hermes's tool_use_enforcement is prompt-level only; this sets the API param
            # so MiniMax cannot drift to prose. Activated when:
            #   - agent.tool_use_enforcement is true (or matches model substring)
            #   - environment variable HERMES_FORCE_TOOL_CHOICE=required is set
            import os as _os
            _force = _os.environ.get("HERMES_FORCE_TOOL_CHOICE")
            if _force in ("required", "auto", "any"):
                api_kwargs["tool_choice"] = _force
            elif self._tool_use_enforcement is True:
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        # v7.55 KAIROS: MiniMax XML tool_call normalizer.
        # MiniMax-M2/M2.5/M2.7 emits tool calls as <minimax:tool_call><invoke name=...>
        # XML embedded in `content` instead of the structured tool_calls[] field.
        # Convert to proper tool_call objects so the existing flow handles them.
        if (not assistant_message.tool_calls) and assistant_message.content:
            _v755_content = assistant_message.content
            if "<minimax:tool_call>" in _v755_content or "<invoke name=" in _v755_content:
                import re as _v755_re
                import json as _v755_json
                from types import SimpleNamespace as _v755_NS
                _v755_blocks = _v755_re.findall(
                    r"<minimax:tool_call>(.*?)</minimax:tool_call>",
                    _v755_content, flags=_v755_re.DOTALL,
                )
                if not _v755_blocks:
                ...
                _v755_synth = []
                for _v755_i, _v755_block in enumerate(_v755_blocks):
                    _v755_name_m = _v755_re.search(r'<invoke name="([^"]+)"', _v755_block)
                    if not _v755_name_m:
                        continue
                    _v755_name = _v755_name_m.group(1)
                    _v755_params = {}
                    for _v755_pm in _v755_re.finditer(
                        r'<parameter name="([^"]+)">(.*?)</parameter>',
                        _v755_block, flags=_v755_re.DOTALL,
                    ):
                        _v755_params[_v755_pm.group(1)] = _v755_pm.group(2)
                    _v755_args = _v755_json.dumps(_v755_params)
                    _v755_call_id = self._deterministic_call_id(_v755_name, _v755_args, _v755_i)
                    _v755_synth.append(_v755_NS(
                        id=_v755_call_id,
                        call_id=_v755_call_id,
                        type="function",
                        function=_v755_NS(name=_v755_name, arguments=_v755_args),
                    ))
                if _v755_synth:
                    assistant_message.tool_calls = _v755_synth
                    # Strip XML from content so it does not appear as prose downstream
                    _v755_clean = _v755_re.sub(
                        r"<minimax:tool_call>.*?</minimax:tool_call>",
                        "", _v755_content, flags=_v755_re.DOTALL,
                    ).strip()
                    assistant_message.content = _v755_clean
                    msg["content"] = _v755_clean
                    msg["finish_reason"] = "tool_calls"
                    finish_reason = "tool_calls"
                    logging.info(
                        f"[v7.55] MiniMax XML normalizer extracted {len(_v755_synth)} tool_calls "
                        f"from content (names={[t.function.name for t in _v755_synth]})"
                    )
