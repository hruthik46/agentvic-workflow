...
_TEMPLATES = {
    'ARCH-DESIGN': {
        ...
        'steps': [
            ...
            'bash: agent send orchestrator "[ARCH-COMPLETE] {gid} iteration {it}"  # must use: agent send (NOT agent msg send — that fails)',
        ],
        'tail_schema': None,
        'rules': [
    ...
    'ARCH-BLIND-REVIEW': {
        ...
        'steps': [
            ...
            'bash: agent send orchestrator "[ARCH-REVIEWED] {gid} iteration {it}"  # dispatcher reads review.json from disk — do NOT pipe JSON',
        ],
    ...
    'E2E-REVIEW': {
        ...
        'steps': [
            ...
            'bash: agent send orchestrator "[E2E-RESULTS] {gid} iteration {it}"  # dispatcher reads e2e-results.json from disk',
        ],
    ...
    'TEST-RUN': {
        'intro': 'TASK: Functional test execution for {gid} iter {it}. Honest pass/fail counts. MUST include live endpoint probes.',
        'steps': [
            'bash: find /var/lib/karios/iteration-tracker/{gid}/phase-2-arch-loop -name "test-cases.md" | sort | tail -1 | xargs cat 2>/dev/null || echo "no test-cases.md found"',
            'bash: cd /root/karios-source-code/{repo} && go build ./... 2>&1 | head -20',
            ...
            'bash: for node in 192.168.118.105 192.168.118.106 192.168.118.2; do echo "=== $node ==="; HTTP_CODE=$(curl -s -o /tmp/probe_${node}.json -w "%{http_code}" http://$node:8089/api/v1/ping 2>&1); echo "HTTP $HTTP_CODE"; cat /tmp/probe_${node}.json 2>/dev/null; echo; done',
            'bash: for endpoint in /api/v1/healthz /; do echo "=== $endpoint ==="; curl -s -o /dev/null -w "%{http_code}" http://192.168.118.106:8089$endpoint; echo; done',
            'write_file: /var/lib/karios/iteration-tracker/{gid}/phase-4-testing/iteration-{it}/test-results.json with {{rating, recommendation, summary, critical_issues, test_results{{passed,failed,skipped}}, evidence{{live_api_probes:[{{node,url,status_code,body,passed}}], build, go_test, go_vet}}, trace_id}}',
            'bash: agent send orchestrator "[TEST-RESULTS] {gid} iteration {it}"',
        ],
        'tail_schema': None,
        'rules': [
            'MANDATORY: evidence.live_api_probes MUST contain curl results from at least 192.168.118.106:8089. Omitting this causes v7.50 gate rejection.',
            ...
            'Only evaluate endpoints declared in the architecture doc for this gap — do not penalise pre-existing unrelated issues.',
            'DO NOT WRITE PROSE. Every output MUST be a tool call.',
        ],
    },
    'PRODUCTION': {
